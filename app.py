...
app.config['SECRET_KEY'] = b'\xd4\x9d\xaf\x8b\xc0\x14Z\xe5\xfb]\xa5!\xe26\xad\xb5'#デモ用
...
def search():
    ...
    if request.method == 'POST':
        ...
        if keyword != '' and pref != '' and industry != '':
            ...
            err_msg = "検索結果が見つかりませんでした"
            store_all = Store.query.filter(Store.pref.like(pref_filter), Store.industry == industry).all()
            store_count = Store.query.filter(Store.pref.like(pref_filter), Store.industry == industry).count()
...
def info():
    ...
    err_msg = "店舗を登録してください"
    store_all = Store.query.order_by(Store.created_at.desc()).limit(5).all()
